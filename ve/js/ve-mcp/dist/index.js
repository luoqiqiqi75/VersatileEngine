...
const builtinPingToolDef = {
    name: VE_PING_TOOL,
    description: "Probe VE HTTP at GET /api/cmd (no VE command). Returns ok + command count if up; use to see whether the service is running.",
    inputSchema: {
        type: "object",
...
async function listCommands() {
    const payload = await httpJson("/api/cmd");
    if (!Array.isArray(payload.commands)) {
        return [];
    }
    return payload.commands.filter((c) => !!c?.name);
}
async function pingVe() {
    const path = "/api/cmd";
    try {
...
function commandBodyForHttp(args) {
    if (args == null) {
        return [];
    }
...
async function callCommand(name, args) {
    return httpJson(`/api/cmd/${encodeURIComponent(name)}`, {
        method: "POST",
