...
XService::XService(QObject *parent) : Module("ve.qt.service.client"), _p(new Private(this))
{
...
void XService::init()
{
    ...
    QObject::connect(mobj()->c("timeout"), &imol::ModuleObject::changed, mobj(), [this](const QVariant& var, const QVariant&, QObject*) {
        _p->timeout = var.toInt();
    });
    QObject::connect(mobj()->r("trigger.connect"), &imol::ModuleObject::changed, mobj(), [this](const QVariant&, const QVariant&, QObject*) {
        if (_p->nobj) _p->nobj->connectToHost(mobj()->r("host.ip")->getString(), mobj()->r("host.port")->getInt());
    });
    QObject::connect(mobj()->r("trigger.disconnect"), &imol::ModuleObject::changed, mobj(),
        [this](const QVariant&, const QVariant&, QObject*) {
            if (_p->nobj) {
                _p->nobj->disconnectFromHost();
            }
        });
}
