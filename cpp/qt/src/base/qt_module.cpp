...
Node* qtConfigNode()
{
    ...
    return r->resolve("ve/entry/module/ve.qt/config");
}
...
static void installQtMessageHandlerIfNeeded(Node* cfg)
{
    bool on = true;
    if (cfg) {
        if (Node* n = cfg->resolve("log/install_qt_message_handler")) {
            on = n->get<bool>(true);
        }
    }
    if (!on) {
...
void applyEarlySettings()
{
    ...
    if (cfg) {
        if (Node* n = cfg->resolve("attr/ShareOpenGLContexts")) {
            share_gl = n->get<bool>(true);
        }
        if (Node* n = cfg->resolve("attr/Use96Dpi")) {
            use_96 = n->get<bool>(QCoreApplication::testAttribute(Qt::AA_Use96Dpi));
        }
    }
    ...
    if (Node* env = cfg->resolve("env")) {
        for (Node* ch : *env) {
            const std::string& k = ch->name();
            if (k.empty()) {
                continue;
            }
            std::string v = ch->get<std::string>("");
            qputenv(k.c_str(), QByteArray(v.data(), int(v.size())));
        }
    }

    if (Node* prep_root = cfg->resolve("path/prepend")) {
#if defined(Q_OS_WIN)
        ...
        for (Node* path_n : *prep_root) {
            QString env_path = QString::fromStdString(path_n->get<std::string>(""));
...
protected:
    void init() override
    {
        ...
        if (cfg) {
            if (Node* n = cfg->resolve("log/bridge_imol_log_signals")) {
                bridge_signals = n->get<bool>(true);
            }
        }
        if (bridge_signals) {
        ...
        if (cfg) {
            if (Node* n = cfg->resolve("log/bridge_imol_log_export")) {
                bridge_export = n->get<bool>(true);
            }
        }
        if (bridge_export) {
...
#else
        if (cfg && qApp) {
            if (Node* n = cfg->resolve("rescue/record_ui_events")) {
                if (n->get<bool>(false)) {
