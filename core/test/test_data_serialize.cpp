...
VE_TEST(yaml_int_roundtrip) {
    AnyData<int> d(42);
    d.control(AbstractData::CHANGEABLE);
    YAML::Node n = d.toYaml();
    VE_ASSERT_EQ(n.as<int>(), 42);

    AnyData<int> d2(0);
    d2.control(AbstractData::CHANGEABLE);
    VE_ASSERT(d2.fromYaml(n));
    VE_ASSERT_EQ(d2.get(), 42);
}

VE_TEST(yaml_string_roundtrip) {
    AnyData<std::string> d("hello");
    YAML::Node n = d.toYaml();
    VE_ASSERT_EQ(n.as<std::string>(), "hello");
}

VE_TEST(yaml_double_roundtrip) {
    AnyData<double> d(3.14);
    d.control(AbstractData::CHANGEABLE);
    YAML::Node n = d.toYaml();
    VE_ASSERT_NEAR(n.as<double>(), 3.14, 0.001);

    AnyData<double> d2(0.0);
    d2.control(AbstractData::CHANGEABLE);
    d2.fromYaml(n);
    VE_ASSERT_NEAR(d2.get(), 3.14, 0.001);
}

VE_TEST(yaml_void_toYaml_isNull) {
    AnyData<void> d;
    YAML::Node n = d.toYaml();
    VE_ASSERT(n.IsNull());
}
