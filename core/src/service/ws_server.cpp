// ws_server.cpp — ve::WsServer: WebSocket transport
//
// Dual-mode protocol:
...
static JsonCommand parseJsonCommand(const std::string& raw)
{
    ...
    if (parsed.isDict()) {
        ...
        if (dict.has("id"))    r.id    = json::stringify(dict["id"]);
    }
    return r;
}
...
struct WsServer::Private
{
    ...
    void textTrigger(SP& sp, uint64_t sid, const MozLine& m) {
        ...
        Node* target = root->resolve(m.key);
        if (!target) {
            sp->async_send("{\"requestId\":" + std::to_string(rid) + ",\"error\":\"not found\"}");
            return;
        }
    ...
    void textCommand(SP& sp, uint64_t /*sid*/, const MozLine& m) {
        Var paramVar = m.param.empty() ? Var() : json::parse(m.param);
        ...
        if (result.isSuccess() || result.isAccepted()) {
            std::string data = json::stringify(result.content());
            sp->async_send("{\"id\":" + idStr
                + ",\"c\":{" + json::stringify(Var(m.key)) + ":" + data + "}}");
        } else {
            sp->async_send("{\"id\":" + idStr
                + ",\"error\":" + json::stringify(Var(result.toString())) + "}");
        }
    }
    ...
    void jsonMessage(SP& sp, uint64_t sid, const std::string& msg) {
        ...
        if (cmd.cmd == "get") {
            ...
            Node* target = root->ensure(cmd.path);
            if (!target) {
    ...
    _p->subscribeSvc->setPushCallback(
        [this](uint64_t sessionId, const std::string& path, const Var& value) {
            ...
            if (_p->isTextSession(sessionId)) {
                pushMsg = "{\"key\":" + json::stringify(Var(path))
