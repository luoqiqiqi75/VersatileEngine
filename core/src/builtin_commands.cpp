// builtin_commands.cpp — VE built-in terminal commands registered via command::reg()
//
...
static Node* resolveTarget(const Flags& f, int posIdx = 0)
{
    ...
    return root->resolve(path, false);
}
...
void initBuiltins()
{
    ...
    reg("ls", [](const Var& input) -> Result {
        ...
        for (int i = 0; i < total; ++i) {
            ...
            if (detailed) {
                out += "  (" + std::string(varTypeName(c->value().type())) + ")";
    ...
    reg("get", [](const Var& input) -> Result {
        ...
        if (f.has("type", 't')) {
            if (target->hasValue())
                RET_OK(std::string(varTypeName(target->value().type())) + "\n");
            RET_OK("(no value)\n");
        }
        if (target->hasValue()) {
            auto& v = target->value();
    ...
    reg("set", [](const Var& input) -> Result {
        ...
        RET_OK("set: " + varPreview(target->value()) + "  (" + varTypeName(target->value().type()) + ")\n");
    ...
    reg("mv", [](const Var& input) -> Result {
        ...
        auto* src = ve::node::root()->resolve(srcPath, false);
    ...
    reg("mk", [](const Var& input) -> Result {
        ...
        auto* n = target->ensure(path);
        if (n) RET_OK("ensured: /" + n->path(ve::node::root()) + "\n");
        RET_FAIL("failed");
    ...
    reg("find", [](const Var& input) -> Result {
        ...
        auto* n = target->resolve(path, useShadow);
    ...
    reg("json", [](const Var& input) -> Result {
        ...
        if (f.has("import")) {
            ...
            if (json::importTree(target, content))
    ...
    reg("child", [](const Var& input) -> Result {
        ...
        if (f.has("key")) {
            ...
            auto* c = target->resolve(childPath);
    ...
    reg("shadow", [](const Var& input) -> Result {
        ...
        if (f.has("set")) {
            ...
            auto* s = ve::node::root()->resolve(setPath, false);
